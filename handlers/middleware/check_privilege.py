...
CHECK_COMMANDS = set(
    privilege.get('scope') for privilege in PRIVILEGES
)
...
class CheckPrivilege(BaseMiddleware):
    ...
    async def on_process_message(self, message: types.Message, *_) -> None:
        ...
        if text in CHECK_COMMANDS:
            if chat_type in ('group', 'supergroup'):
                await self._check_group_privilege(chat_id, user_id, text)
            elif chat_type == 'private':
                await self._check_user_privilege(user_id, text)
    ...
    def _check_command_privilege(self, privilege: str, command: str) -> bool:
        ...
        if data and command in data[privilege]['scope']:
            return True
        return False
